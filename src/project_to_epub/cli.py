...
def main(
    input_directory: Annotated[
        Path, 
        typer.Argument(help="Path to the project directory to convert")
    ],
    output: Annotated[
        Optional[Path], 
        typer.Option("-o", "--output", help="Output EPUB file path")
    ] = None,
    theme: Annotated[
        Optional[str],
    ...
    author: Annotated[
        Optional[str],
        typer.Option(help="Set EPUB author")
    ] = None,
    limit_mb: Annotated[
        Optional[float],
        typer.Option(help="Set large file threshold in MB")
    ] = None,
    no_skip_large: Annotated[
        bool,
        typer.Option(help="Error out on large files instead of skipping")
    ] = False,
    version: Annotated[
        bool,
    ...
    if not input_directory.exists() or not input_directory.is_dir():
        typer.echo(f"Error: Input directory '{input_directory}' does not exist or is not a directory", err=True)
        raise typer.Exit(code=1)
