...
class Project:
    ...
    def scan_files(self) -> List["FileEntry"]:
        ...
        large_file_threshold = self.config.get("large_file_threshold_mb", 10) * 1024 * 1024
                    ...
                    try:
                        ...
                        if file_size > large_file_threshold:
                            if self.config.get("skip_large_files", True):
                                ...
                                logger.warning(f"Including large file ({file_size / 1024 / 1024:.2f} MB): {file_path}")
                    except Exception as e:
    ...
    def get_file_content(self, file_entry: "FileEntry") -> Optional[str]:
        ...
        """
        try:
            with open(file_entry.absolute_path, 'r', encoding='utf-8', errors='replace') as f:
                return f.read()
        except Exception as e:
...
def highlight_code(content: str, lexer, formatter: HtmlFormatter) -> str:
    ...
    try:
        ...
        if not highlighted.strip():
            logger.warning(f"Pygments returned empty highlighted content for {lexer.name}")
...
def organize_toc_items_by_directory(toc_items: List[Dict[str, str]]) -> List[Dict]:
    ...
    result = [item for item in toc_items if not item.get("href", "").startswith("file_")]
...
<body>
    ...
    def add_toc_items(items, level=0):
        ...
        for item in items:
            ...
            if item.get("is_directory", False) and "children" in item and item["children"]:
                nav_content.append(f"{indent}<li><span class='toc-directory'>{item['title']}</span>")
            ...
            # Regular file item
                ...
                nav_content.append(f'{indent}<li><a href="{item["href"]}">{item["title"]}</a></li>')
...
<body>
    ...
    {html_content}
        ...
        with open(epub_dir / "content.opf", "w", encoding="utf-8") as f:
            ...
            manifest_items.append(f'<item id="ncx" href="{ncx_file}" media-type="application/x-dtbncx+xml"/>')
            manifest_items.append(f'<item id="nav" href="{nav_file}" media-type="application/xhtml+xml" properties="nav"/>')
